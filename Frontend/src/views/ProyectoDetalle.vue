    ...
    async loadComparisonData() {
      ...
      try {
        ...
        const [availabilityData, costData, timeData, qualityData] = await Promise.all([
          AssignmentService.previewBasicAssignment(projectId).catch(err => {
            console.error('Error cargando disponibilidad:', err);
            return { error: err.response?.data?.error || err.message || 'Error desconocido' };
          }),
          AssignmentService.previewCostAssignment(projectId).catch(err => {
            console.error('Error cargando costo:', err);
            return { error: err.response?.data?.error || err.message || 'Error desconocido' };
          }),
          AssignmentService.previewTimeAssignment(projectId).catch(err => {
            console.error('Error cargando tiempo:', err);
            return { error: err.response?.data?.error || err.message || 'Error desconocido' };
          }),
          AssignmentService.previewQualityAssignment(projectId).catch(err => {
