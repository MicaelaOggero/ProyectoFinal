...
- Dentro cada lista de desarrolladores para cada tarea, seleccionar al que tengan la mejor puntuacionPromedioCalidad y feedbackHistorico, ya que esto indica que va a completar la tarea con más calidad. En caso de empate, elegir teniendo en cuenta la disponibilidad (elegir el que tenga mas disponibilidad), los años de experiencia y las preferencias de habilidades (preferenciasHabilidad) del desarrollador, priorizando aquellos que tengan mejor puntuación promedio en las habilidades requeridas por la tarea. (NO SELECCIONAR UN DESARROLLADOR QUE NO ESTE EN LA LISTA DE CANDIDATOS DE LA TAREA)

2) En en el caso de que la tarea no tenga desarrolladoresCandidatos ("sinCandidatos": true), no asignar ningún desarrollador y en el json que se pide solo mostrar los siguientes campos en el formato que corresponde al json para esa tarea:
  id: tarea._id,
  nombre: tarea.nombre,
  descripcion: tarea.descripcion,
  fechaEstimadaInicio: tarea.fechaEstimadaInicio,
