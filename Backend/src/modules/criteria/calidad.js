...
- Calidad de las tareas (puntuacionPromedioCalidad)
- Calidad de los proyectos (feedbackHistorico)

Reglas de asignación:

1) PRIMER FILTRO (OBLIGATORIO)
- Dentro cada lista de desarrolladoresCandidatos (mientras no esté vacía) para cada tarea, seleccionar al que tengan la mejor puntuacionPromedioCalidad y feedbackHistorico (haciendo un promedio entre ambas), ya que esto indica que va a completar la tarea con más calidad. (NO SELECCIONAR UN DESARROLLADOR QUE NO ESTE EN LA LISTA desarrolladoresCandidatos DE LA TAREA). Si desarrolladoresCandidatos es vacía, no asignar la tarea.

2) SEGUNDO FILTRO - En caso de empate, elegir teniendo en cuenta la cantidad de veces que el desarrollador fue clasificado (puntuacionPromedioCalidad.tareasCalificadas y feedbackHistorico.vecesCalificado). En caso de otro empate, elegir teniendo en cuenta la disponibilidad (elegir el que tenga mas disponibilidad), los años de experiencia y las preferencias de habilidades (preferenciasHabilidad) del desarrollador.

3) MUY IMPORTANTE: solo se deben tener en cuenta para la selección la puntuacionPromedioCalidad y feedbackHistorico del desarrollador como prioridad y luego (en caso de empate) la disponibilidad, preferencias, años de experiencia, las habilidades requeridas de la tarea y las habilidades del desarrollador. No considerar ninguno de los otros factores, costo por hora y rendimiento histórico para la selección del desarrollador. Es muy importante distribuir las horas de forma uniforme entre los dias disponibles.
...
export async function confirmarAsignacionPorCalidad(projectId, sugerencias) {
  ...
  return {
    message: "Asignaciones confirmadas y simulación guardada (criterio calidad).",
    resultados
  };
}
