...
export const previewObtenerAsignacionesPorTiempoIA = async (projectId) => {
  ...
  if (!tareas.length)
    throw new Error("No hay tareas sin asignarpara este proyecto");
...
- RendimientoHistorico del desarrollador (rendimientoHistorico)

Reglas de asignación:

1) PRIMER FILTRO (OBLIGATORIO)
- Dentro cada lista de desarrolladoresCandidatos (mientras no esté vacía) para cada tarea, seleccionar al que tengan el MAYOR rendimientoHistorico.promedioPorcentaje (el más rápido históricamente). Se busca que la tarea pueda completarse en el menor tiempo posible. (NO SELECCIONAR UN DESARROLLADOR QUE NO ESTE EN LA LISTA desarrolladoresCandidatos DE LA TAREA). Si desarrolladoresCandidatos es vacía, no asignar la tarea.

2) SEGUNDO FILTRO - En caso de empate en el rendimientoHistorico.promedioPorcentaje, elegir al que tenga la mayorcantidad de tareas completadas (rendimientoHistorico.tareasCompletadas). En caso de otro empate, elegir teniendo en cuenta la disponibilidad (elegir el que tenga mas disponibilidad), los años de experiencia (aniosExperiencia) y las preferencias de habilidades (preferenciasHabilidad) del desarrollador.

3) MUY IMPORTANTE: solo se deben tener en cuenta para la selección el rendimientoHistorico.promedioPorcentaje del desarrollador y luego (en caso de empate) la disponibilidad, preferencias, años de experiencia. No considerar ninguno de los otros factores como, costo por hora, calidad o feedback histórico para la selección del desarrollador. Es muy importante distribuir las horas de forma uniforme entre los dias disponibles. 
...
Devuelve un JSON **válido** con esta estructura (Nada más que el JSON):
{
  ...
  (si o si, si la tarea NO tuvo desarrolladoresCandidatos)
    {
      tareaId: tarea._id,
      nombre: tarea.nombre,
      descripcion: tarea.descripcion,
